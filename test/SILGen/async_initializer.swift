...
struct MyStruct {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers8MyStructVACyYacfC : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @thin MyStruct.Type) -> MyStruct {
  init() async {}
}

enum MyEnum {
  // NI-DAG: sil hidden [ossa] @$s12initializers6MyEnumOACyYacfC : $@convention(method) @async (@thin MyEnum.Type) -> MyEnum
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers6MyEnumOACyYacfC : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @thin MyEnum.Type) -> MyEnum {
  init() async {}
}
...
actor MyActor {
  ...
  // NI-NS-NEXT: [[ISOLATION_1_B_CAST:%.*]] = unchecked_value_cast [[ISOLATION_1_B]] : $Optional<any Actor> to $Builtin.ImplicitActor
  // NI-NS-NEXT: // function_ref
  // NI-NS-NEXT: [[FN:%.*]] = function_ref @$s12initializers8MyStructVACyYacfC :
  // NI-NS-NEXT: apply [[FN]]([[ISOLATION_1_B_CAST]]
...
class EarthPerson : Person {
  // NI-NS-LABEL: sil hidden [exact_self_class] [ossa] @$s12initializers11EarthPersonCACyYacfC : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @thick EarthPerson.Type) -> @owned EarthPerson {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers11EarthPersonCACyYacfc : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @owned EarthPerson) -> @owned EarthPerson {
  required init() async {}
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers11EarthPersonC4nameACSS_tYacfc : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @owned String, @owned EarthPerson) -> @owned EarthPerson {
  ...
  // NI-NS-DAG: sil private [transparent] [thunk] [ossa] @$s12initializers11EarthPersonCAA0C0A2aDPxyYacfCTW : $nonisolated(nonsending) @convention(witness_method: Person) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @thick EarthPerson.Type) -> @out EarthPerson {
}
...
class NorthAmericaPerson : EarthPerson {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers18NorthAmericaPersonCACyYacfc : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @owned NorthAmericaPerson) -> @owned NorthAmericaPerson {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers18NorthAmericaPersonC4nameACSS_tYacfc : $nonisolated(nonsending) @convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @owned String, @owned NorthAmericaPerson) -> @owned NorthAmericaPerson {
...
class Cat {
  ...
  // NI-NS:          {{%[0-9]+}} = apply {{%[0-9]+}}({{%.*}}) : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> ()
...
// NI-NS-LABEL:  sil hidden [ossa] @$s12initializers7makeCatyyYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> () {
// NI-NS:        bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor):
// NI-NS:          hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers7makeDogyyYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> () {
// NI-NS: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor):
// NI-NS:      hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL:  sil hidden [ossa] @$s12initializers8makeBirbyyYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> () {
// NI-NS:        bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor):
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers20makeActorFromGenericAA04SomeC0CyYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> @owned SomeActor {
// NI-NS:      bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor):
// NI-NS:          hop_to_executor [[ACTOR]]
// NI-NS:          apply {{%.*}}({{%.*}})
// NI-NS-NEXT:     hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers26callActorMethodFromGeneric1ayAA04SomeC0C_tYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @guaranteed SomeActor) -> () {
...
// NI-NS-LABEL: sil hidden{{.*}} [ossa] @$s12initializers15makeActorInTaskyyYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor) -> () {
// NI-NS:       bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor):
// NI-NS:          hop_to_executor [[ACTOR]]
// NI-NS:          apply
...
// NI-NS-LABEL: sil hidden{{.*}} [ossa] @$s12initializers21callActorMethodInTask1ayAA04SomeC0C_tYaF : $nonisolated(nonsending) @convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitActor, @guaranteed SomeActor) -> () {
// NI-NS: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitActor, [[ARG:%.*]] : @guaranteed $SomeActor):
// NI-NS:   hop_to_executor [[ACTOR]]
