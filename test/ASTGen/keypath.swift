...
// RUN: DUMP_ARGS="-dump-ast -dump-ast-format default-with-decl-contexts"
// RUN: %target-swift-frontend %s ${DUMP_ARGS} -enable-experimental-feature ParserASTGen -verify \
// RUN:   | %utils/sanitize-address.py > %t/astgen.ast
// RUN: not %target-swift-frontend %s ${DUMP_ARGS} \
