...
// RUN: %target-swift-frontend %s ${DUMP_ARGS} -disable-availability-checking -enable-experimental-move-only -enable-experimental-feature ThenStatements -enable-experimental-feature ParserASTGen \
// RUN:    | %utils/sanitize-address.py > %t/astgen.ast
// RUN: %target-swift-frontend %s ${DUMP_ARGS} -disable-availability-checking -enable-experimental-move-only -enable-experimental-feature ThenStatements \
