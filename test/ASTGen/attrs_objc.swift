...
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) %s ${DUMP_ARGS} -import-objc-header %S/Inputs/objc_decls.h -enable-experimental-feature ParserASTGen \
// RUN:   | %utils/sanitize-address.py > %t/astgen.ast
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) %s ${DUMP_ARGS} -import-objc-header %S/Inputs/objc_decls.h \
