...
// RUN: %target-swift-frontend %s ${DUMP_ARGS} -target %target-swift-5.1-abi-triple -enable-experimental-move-only -enable-experimental-feature ParserASTGen \
// RUN:   | %utils/sanitize-address.py > %t/astgen.ast
// RUN: %target-swift-frontend %s ${DUMP_ARGS} -target %target-swift-5.1-abi-triple -enable-experimental-move-only \
