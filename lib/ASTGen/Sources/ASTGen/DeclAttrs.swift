...
extension ASTGenVisitor {
  ...
  func generateDerivativeOriginalDecl(expr: ExprSyntax) -> GeneratedDerivativeOrignalDecl? {
    var baseType: BridgedTypeRepr?
    var declName: BridgedDeclNameRef
    var declNameLoc: BridgedDeclNameLoc
    ...
    return GeneratedDerivativeOrignalDecl(
      baseType: baseType,
      declName: declName,
  ...
  func generateDerivativeAttr(attribute node: AttributeSyntax) -> BridgedDerivativeAttr? {
    ...
    if let accessorKind {
      return .createParsed(
        ...
        orignalName: originalDecl.declName,
