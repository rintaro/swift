...
namespace {
using TypeAndDecl = std::pair<AnyFunctionType *, ValueDecl *>;

static void collectPossibleCalleeByQualifiedLookup(
    DeclContext &DC, Type baseTy, DeclBaseName name,
    SmallVectorImpl<TypeAndDecl> &candidates) {
...
void collectPossibleCalleeByQualifiedLookup(
  ...
  collectPossibleCalleeByQualifiedLookup(DC, baseTy, name, candidates);
}

bool collectPossibleCalleeApply(
  ...
  if (auto type = fnExpr->getType()) {
    ...
    collectPossibleCalleeByQualifiedLookup(
...
bool collectPossibleCalleeSubscript(
  ...
  if (subscriptExpr->hasDecl()) {
    ...
    collectPossibleCalleeByQualifiedLookup(DC, subscriptExpr->getBase(),
...
class CodeCompletionTypeContextAnalyzer {
  ...
  SmallVector<TypeAndDecl, 2> PossibleCallee;
  ...
  static bool translateArgIndexToParamIndex(Expr *Args, unsigned &Position,
                                            bool &HasName) {
  ...
  collectArgumentExpectation(DeclContext &DC, Expr *E, Expr *CCExpr) {
    ...
    SmallVector<TypeAndDecl, 4> Candidates;
    Expr *Arg = nullptr;
    if (auto *applyExpr = dyn_cast<ApplyExpr>(E)) {
      if (!collectPossibleCalleeApply(DC, applyExpr, Candidates))
        return false;
      Arg = applyExpr->getArg();
    } else if (auto *subscriptExpr = dyn_cast<SubscriptExpr>(E)) {
      if (!collectPossibleCalleeSubscript(DC, subscriptExpr, Candidates))
        return false;
      Arg = subscriptExpr->getIndex();
    }
    // Record possible callee info.
    for (auto &typeAndDecl : Candidates)
      PossibleCallee.push_back(typeAndDecl);
...
public:
  ...
  ArrayRef<TypeAndDecl> getPossibleCallee() const { return PossibleCallee; }
  ...
  case CompletionKind::PostfixExprParen: {
    ...
    if (ShouldCompleteCallPatternAfterParen) {
      if (ExprType) {
        ...
        for (auto &typeAndDecl : TypeAnalyzer.getPossibleCallee())
